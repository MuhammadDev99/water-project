...
> In this template, we use `@water` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@water` to something like `@my-company` or `@project-name`.
...
pnpm --filter @water/auth generate
```
...
3. Outputs a Drizzle-compatible schema file to the `@water/db` package
